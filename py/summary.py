...
class Summary(tk.Frame):
	...
	def main_frame(self):
		...
		self.krep_button = ttk.Button(
			show_plot_frame, text="Show", command=lambda: self.show_krep())
		self.krep_button.grid(row=0, column=0, padx=45, pady=2)

		self.stroop_button = ttk.Button(
			show_plot_frame, text="Show", command=lambda: self.show_stroop())
		self.stroop_button.grid(row=0, column=1, padx=45, pady=2)

		self.rotation_button = ttk.Button(
			show_plot_frame, text="Show", command=lambda: self.show_rotation())
		self.rotation_button.grid(row=0, column=2, padx=45, pady=2)
	...
	def show_krep(self):
		...
		if len(self.controller.krep_answer_total) == 0:
			...
			good_answers = tk.Label(
				stats_frame, text=reformat_good_answ, justify='left', font=LARGE_FONT)
	...
	def show_stroop(self):
		...
		if self.controller.stroop_counter == 0:
			...
			reformat_fast_ncong = ("Fastest time of reaction (noncongruent): "
				+str(round(min(self.controller.stroop_time_stamps_noncong), 4))
				+" sec")
			...
			reformat_slow_ncong = ("Slowest time of reaction (noncongruent): "
				+str(round(max(self.controller.stroop_time_stamps_noncong), 4))
				+" sec")
			...
			reformat_avg_ncong = ("Average time of reaction (noncongruent): "
				+str(round(mean(self.controller.stroop_time_stamps_noncong), 4))
				+" sec")
			...
			a1.plot(range(1, self.controller.stroop_counter+1),
			...
			a2.plot(range(1, (len(self.controller.stroop_time_stamps_cong)+1)),
			...
			a3.plot(range(1, (len(self.controller.stroop_time_stamps_noncong)+1)),
